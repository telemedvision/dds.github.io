...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const validationRules = {
        ...
        ...Array.from({length: 17}, (_, i) => ({
            [`dds-${i + 1}`]: {
                required: true,
                message: `Please answer DDS question ${i + 1}`
            }
        ...
        'psychosocial-meal-plan': {
            required: true,
            message: 'Please answer the meal plan compliance question'
        }
    };
    ...
    function validateForm() {
        ...
        for (const [fieldName, rule] of Object.entries(validationRules)) {
            ...
            if (rule.required && !field || (field && !field.checked)) {
                errors.push({
                    field: fieldName,
    ...
    function setupRealtimeValidation() {
        ...
        document.querySelectorAll('input[type="radio"]').forEach(radio => {
            radio.addEventListener('change', function() {
                ...
                const fieldsetError = this.closest('fieldset')?.querySelector('.error-message');
    ...
    async function submitFormData(data, submitButton) {
        try {
